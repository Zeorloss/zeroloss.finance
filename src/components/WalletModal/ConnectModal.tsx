...
const ConnectModal = ({ login, logout, onDismiss = () => null, displayCount = 3 }: ConnectModalProps): JSX.Element => {
      ...
      <div className="relative text-xl font-medium text-center mt-2 mb-6 p-4">
        ...
        <button
          ...
          className="absolute top-4 right-4 p-1 bg-gray-50/40 inline-block rounded-full
            hover:bg-gray-50/90 hover:text-primary cursor-pointer"
        >
      ...
      <div className="grid grid-cols-3 gap-4 mb-3">
        {displayListConfig.map((wallet) => (
          <div key={wallet.title} className="w-20 inline-block m-2">
            <WalletCard walletConfig={wallet} login={login} onDismiss={onDismiss} isConnecting={isConnecting} />
          </div>
        ))}
